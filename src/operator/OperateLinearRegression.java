...
import linearregression.DataObject;
import linearregression.ListLearningEngine;
import linearregression.ReadFiles;

/**
 *
 * @author Welcome
 */
public class OperateLinearRegression implements Runnable{
 private static final Double THETA_LR=0.0;
    private static final Double COST_ERROR_LR=0.0;
     private static final Double LEARNING_RATE_LR=0.001;
    ...
    public void run() {
      ...
      System.out.println("Pattern prediction using linear regression\n");
      ...
      for(int j=0;j<data.getInputs().size();j++){
         ...
         System.out.println("Predicting values ("+data.getTargetParameter()+"): "+ engine.predict(input, input.size()));
